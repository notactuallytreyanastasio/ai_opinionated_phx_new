...
This separation makes the codebase testable, serializable, and ready to extract into different execution contexts (GenServer with PubSub, distributed systems, etc.) without rewriting logic.

```
# Functional core -- pure functions on structs
Engine.tick(game_state)       # GameState -> GameState
Board.find_matches(board)     # Board -> Board
Piece.rotate(piece)           # Piece -> Piece

# Imperative shell -- orchestrates side effects
def handle_info(:tick, socket) do
  game = Engine.tick(socket.assigns.game)  # call into pure core
  {:noreply, assign(socket, game: game)}   # side effect: update LiveView
end
```
...
defmodule MyApp.Game.Board do
  @moduledoc "NxM grid of cells with collision detection and gravity."

  @type cell :: :empty | :dark | :light | :marked_dark | :marked_light
  @type t :: %__MODULE__{
  ...
  def place_piece(board, piece, position) do
    # Implementation follows naturally from the types
  end
end
```

Types make boundaries visible. When module A consumes `Board.t()` and module B produces `Board.t()`, the contract is explicit. No runtime surprises, no "what shape is this data?" debugging.
...
- `make quality` -- **always use this** before committing (compile, credo, test, dialyzer, improve-elixir, format)
...
## Subagents MUST Log to Deciduous Themselves

When launching subagents in git worktrees, **the subagent must log to deciduous from within its worktree**. The main session CANNOT log on behalf of a worktree because deciduous auto-tags nodes with the current git branch. Logging retroactively from a different branch breaks the graph.
...
Pass the parent node ID into the subagent prompt so it can link correctly.
